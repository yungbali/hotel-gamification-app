...
export class StorageService {
  ...
  async initializeDemoData(): Promise<void> {
    const versionKey = 'demo_data_version';
    const storedVersion = await this.getItem<string>(versionKey);

    if (storedVersion !== StorageService.DEMO_DATA_VERSION) {
    ...
    if (existingUsers.length === 0) {
      ...
      const demoUsers: User[] = [
        ...
        } as Waiter,
        {
          ...
          isActive: true,
          createdAt: new Date(),
          permissions: ['view_analytics', 'manage_staff', 'resolve_complaints'],
        } as Manager,
        {
          id: '3',
          name: 'Mike Wilson',
      ...
      await this.storeHotel(demoHotel);
      
      // Create some demo ratings
      await this.createDemoRatings();

      // Create demo shifts to populate manager dashboards
      await this.createDemoShifts();

      // Ensure no user is auto-logged in after seeding demo data
      await this.removeItem('current_user');
    }

    await this.setItem(versionKey, StorageService.DEMO_DATA_VERSION);
  }
